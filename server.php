...
if(isset($_POST['method'])){
	if(strcmp('inicia-jogo', $_POST['method']) == 0){
		...
		$rodadasRestantes;
		$sql = "select * from jogo where id=$idJogo";
		$result = $conn->query($sql);
		if ($result->num_rows > 0) {
			foreach($result as $model){
				$rodadasRestantes = $model["rodadas_restantes"];
			}
		}else{
			$rodadasRestantes=10;
		}	
		...
		if($rodadasRestantes!=0){
			...
			if ($result->num_rows > 0) {
				...
				if ($conn->query($sql) === TRUE) {
					...
					if ($result->num_rows > 0) {
						...
						$rodadasRestantes = $rodadasRestantes-1;
						$sql = "update jogo set rodadas_restantes = $rodadasRestantes where id=$id_jogo";
						...
						if ($conn->query($sql) === TRUE) {
							...
							array_push($arrayResponse, array('id'=>"erro-ao-decrementar-rodadas [".$conn->error."]"));
							echo json_encode($arrayResponse);
						}
			...
			if ($result->num_rows > 0) {
				...
				array_push($arrayResponse, array('id_rodada'=>'finalizado',
												 'n_rodada'=>'finalizado',
												 'pt_slot_1'=>$pt_j1,
												 'pt_slot_2'=>$pt_j2,
												 'id_jogo'=>$id_jogo));
